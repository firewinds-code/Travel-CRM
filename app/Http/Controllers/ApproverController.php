...
class ApproverController extends Controller
{
            ...
            if ($request->ajax()) {
                $data = User::join('travel_details', 'users.emp_id', 'travel_details.employee_id')
                    ...
                    ->where(function ($query) {
                        $query->where('travel_details.approval_status_by', [Auth::User()->name]);
                        $query->orwhere('travel_details.created_by', [Auth::User()->emp_id]);
                    })
                    ->get();
                return DataTables::of($data)
                    ->make(true);
            }
            return view('approver.report');
        } catch (Exception $e) {
            return back()->with("error", "Something Went Wrong");
        }
    }
    ...
    public function editRequests(Request $request)
    {
        try {
            ...
            $html = view('approver.view', compact('query', 'places', 'process'))->render();
        ...
        try {
            if ($request->ajax()) {
                $data = User::join('travel_details', 'users.emp_id', 'travel_details.employee_id')
                    ...
                    ->where('users.usertype', '=', 3) // Fetch data only when usertype is 'requester'
                    ->get();
                return DataTables::of($data)
                    ->addIndexColumn()
